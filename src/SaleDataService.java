...
public class SaleDataService {
    ...
    public static void creatYearlySalesReport(List<TeslaSales> amount, String model){
        System.out.println(model + " Yearly Sales Report");
        System.out.println("---------------------------");

        Map<Integer, Integer> yearlySales = amount.stream()
                                                 .collect(Collectors.groupingBy
                                                         (teslaSales -> teslaSales.getYearMonth().getYear(),
                                                                 Collectors.summingInt(TeslaSales::getSalesAmount)));
        yearlySales.forEach((a,z)-> System.out.println(a+" -> "+z));
        System.out.println(" ");

    }

    public static void bestAndWorstMonth(List<TeslaSales> amount, String model){
        amount.stream()
                .max(Comparator
                .comparingInt(TeslaSales::getSalesAmount))
                .ifPresent(bestMonth ->
    ...
    public static void generateTeslaSaleReport(FileService read) throws IOException {
        ...
        List<TeslaSales> model3 = read.readFile("model3.csv");
        creatYearlySalesReport(model3, "Model 3");
        bestAndWorstMonth(model3, "Model 3");

        List<TeslaSales> modelS = read.readFile("modelS.csv");
        creatYearlySalesReport(modelS, "Model S");
        bestAndWorstMonth(modelS, "Model S");

        List<TeslaSales> modelX = read.readFile("modelX.csv");
